...
class Strategy:
    def __init__(self, client: Union["BitmexFuturesClient", "BinanceFuturesClient"], contract: Contract, exchange: str,
                 timeframe: str, balance_percentage: float, take_profit: float, stop_loss: float, strat_name):
    ...
    def _check_tp_sl(self, trade: Trade):
        ...
        if trade.side == "long":
            if self.stop_loss is not None:
                if price <= trade.entry_price * (1 - self.stop_loss / 100): #stop-loss exit price for a long trade
                    sl_triggered = True
...
class TechnicalStrategy(Strategy):
    def __init__(self, client, contract: Contract, exchange: str, timeframe: str, balance_percentage: float, take_profit: float,
                 stop_loss: float, other_params: Dict):
        super().__init__(client, contract, exchange, timeframe, balance_percentage, take_profit, stop_loss, "Technical")
    ...
    def _rsi(self): # RS = AVERAGE Gain / AVERAGE Loss
        close_list = []
        ...
        avg_gain = up.ewm(com=(self._rsi_length - 1), min_periods=self._rsi_length).mean() #center of mass
    ...
    def _check_signal(self):
        ...
        if rsi < 30 and macd_line > macd_signal: #rsi < 30 meaning the contract is oversold
            return 1
        elif rsi > 70 and macd_line < macd_signal: #rsi > 70 meaning the contract is overbought
            return -1
        else:
            return 0
...
class BreakoutStrategy(Strategy):
    def __init__(self, client, contract: Contract, exchange: str, timeframe: str, balance_percentage: float, take_profit: float,
                 stop_loss: float, other_params: Dict):
        super().__init__(client, contract, exchange, timeframe, balance_percentage, take_profit, stop_loss, "Breakout")

        self._min_volume = other_params['min_volume']

        # print("Activated strategy for", contract.symbol)

    def _check_signal(self) -> int: #long signal 1; short signal -1 ; 0 no signal
    ...
    def check_trade(self, tick_type: str):
        ...
        if not self.ongoing_position: #tick_type == "new_candle" and
            signal_result = self._check_signal()
            # print("signal result: " + str(signal_result))
            if signal_result in [-1, 1]:
                self._open_position(signal_result)